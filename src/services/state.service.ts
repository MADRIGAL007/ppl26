...
export class StateService {
    ...
    // SECURITY: Removed hardcoded adminUsername/adminPassword - all auth via AuthService
    ...
    private generateId(): string {
        ...
        // Fallback for older browsers - still cryptographically secure
        ...
        console.warn('[Security] crypto.randomUUID not available, using fallback');
    ...
    // REMOVED: loginAdmin - deprecated, use AuthService
    ...
    async changeAdminPassword(oldP: string, newP: string): Promise<boolean> {
        try {
            ...
            if (res.ok) {
                // Password updated server-side, no local state needed
                return true;
            }
        } catch (e) { }
        return false;
