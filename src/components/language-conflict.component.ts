      ...
      <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 font-sans animate-fade-in">
         ...
         <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm relative z-10 overflow-hidden animate-slide-up">
            <div class="p-6 text-center">
                <div class="w-12 h-12 rounded-full bg-blue-50 text-pp-blue flex items-center justify-center mx-auto mb-4">
                    <span class="material-icons">language</span>
                </div>

                <h3 class="text-lg font-bold text-pp-navy mb-2">{{ 'MODAL_LANG.TITLE' | translate }}</h3>
                <p class="text-sm text-slate-600 mb-6 leading-relaxed">
                   {{ 'MODAL_LANG.DESC' | translate: { country: countryName(), deviceLang: deviceLangName() } }}
                </p>

                <div class="space-y-3">
                    <button (click)="selectLang(deviceLangCode())" class="pp-btn">
                        {{ 'MODAL_LANG.USE_DEVICE' | translate: { deviceLang: deviceLangName() } }}
                    </button>

                    <button (click)="selectLang(ipLangCode())" class="w-full py-3 rounded-full font-bold text-pp-blue hover:bg-blue-50 transition-colors text-sm border border-transparent">
