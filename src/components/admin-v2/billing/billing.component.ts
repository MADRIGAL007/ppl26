...
import { calculateTotalPrice } from '../../../services/flows.service';
             ...
             <div class="flex justify-between items-start">
                 <div>
                    ...
                    <p class="text-slate-400 mt-2 max-w-lg">
                        {{ metrics().count }} active flow{{ metrics().count !== 1 ? 's' : '' }} enabled.
                        Unlimited sessions, advanced fingerprinting, and priority support.
                    </p>
                 </div>
                 <div class="text-right">
                    <p class="text-3xl font-bold text-white">
                        {{ metrics().amount | currency:'USD':'symbol':'1.0-0' }}
                        <span class="text-lg text-slate-500 font-normal">/mo</span>
                    </p>
                    <p class="text-xs text-slate-500 mt-1">Next billing: {{ metrics().nextBilling }}</p>
                 </div>
             </div>
          </div>
          <div class="p-8 bg-slate-950/30 flex gap-4">
              <button class="adm-btn adm-btn-primary" (click)="manage()">Manage Subscription</button>
...
export class BillingComponent {
   ...
   // Computed metrics
   metrics = computed(() => {
      const enabled = this.settings.userSettings().enabledFlows || [];
      return {
         amount: calculateTotalPrice(enabled),
         count: enabled.length,
         nextBilling: new Date(new Date().setDate(new Date().getDate() + 30)).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
      };
   });
